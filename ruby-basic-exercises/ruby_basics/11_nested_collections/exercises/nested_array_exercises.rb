def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  # change with it
  final_array = Array.new(number_of_rows) {Array.new(seats_per_row)}
end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.filter! do |row|
    row if row[:is_my_favorite?] == true
  end

  return array_of_hash_objects[0]
